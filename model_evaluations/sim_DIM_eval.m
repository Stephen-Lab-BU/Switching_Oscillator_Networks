...
percEdges = [0.05 0.05 0.05];       %percentage of edges in the network
...
for j=1:numStates
    % Find indices where adjMatrix is 1 
    [edgeRows, edgeCols] = find(adjMatrix(:,:,j) == 1);
    % For each edge, assign a random value
    for i = 1:length(edgeRows)
        magMatrix(edgeRows(i), edgeCols(i),j) = mag_upper - (mag_upper - mag_lower) * rand();
        phaseMatrix(edgeRows(i), edgeCols(i),j) = 2 * pi * rand();
    end
end

% Define A matrix
AMatrix = zeros(numNodes*2, numNodes*2,numStates);
...
for k=1:numStates
    for i = 1:size(magMatrix, 1)
        for j = 1:size(magMatrix, 2)
            ...
            AMatrix(rowStart:rowStart+1, colStart:colStart+1,k) = block;
        end
    end
end

% Update diagonal blocks in combinedMatrix
for k=1:numStates
    for i = 1:size(magMatrix, 1)
        % Calculate the rotation matrix component
        rotationMatrix = [cos(2*pi*osciFreq/fs) -sin(2*pi*osciFreq/fs); sin(2*pi*osciFreq/fs) cos(2*pi*osciFreq/fs)];
        rotationMatrix = aAR * rotationMatrix;
        
        % Compute row sum
        row_sum(i, k) = sum(magMatrix(i, :, k));
        % Calculate the position in the combinedMatrix
        rowStart = (i-1)*2 + 1;
        
        % Update the diagonal block
        AMatrix(rowStart:rowStart+1, rowStart:rowStart+1,k) = rotationMatrix - row_sum(i, k) * eye(2);
    end
end
...
% Simulate the system
for t = 2:N
    currentState = S_t(t); % Get the current state from S_t to decide which A matrix to use
    u_t = sqrt(var_proc_noi) * randn(2*numOsci, 1);    % Process noise
    v_t(:,t) = sqrt(var_obs_noi) * randn(numNodes, 1); % Observation noise
    x(:, t) = AMatrix(:, :, currentState) * x(:, t-1) + u_t; % Use A matrix corresponding to current state
...
%% Theoretical cross-spectrum & coherence for true A
...
sgtitle(sprintf('Coherence: True A (\\alpha=%g, var(R)=%g)', aAR, var_obs_noi))

%plot cross-spectrum matrix - True A
figure
tiledlayout(1,numStates)
for i=1:numStates
    nexttile
...
sgtitle(sprintf('Cross-spectrum: True A (\\alpha=%g, var(R)=%g)', aAR, var_obs_noi))
