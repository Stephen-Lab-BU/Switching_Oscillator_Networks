...
% =========================================================================
% simulation study
% network structure is reflected through the process noise cov matrix (Q)
% use this file for 
...
% ** first, load results from EM on Q **
...
%% Theoretical coherence & cross-spectrum of MLE Q 

% Initialize the coherence matrix for all states
iter = size(mle_Q, 4);
CohMLEQ = zeros(numNodes, numNodes, numStates);
...
for st = 1:numStates
    ...
    CohMLEQ(:, :, st) = abs(coherenceMatrix);
end
true_Coh = Coh;
S_y1y2_7hz_MLEQ = squeeze(S_y1y2(:,:,freqIndex,:));

%plot coherence matrix - MLE Q
figure
tiledlayout(1,numStates)
for i=1:numStates
    nexttile
    n = numNodes;
    imagesc(CohMLEQ(:,:,reorder(i)))
...
sgtitle(sprintf('Coherence: MLE \\Sigma (\\alpha=%g, var(R)=%g)', aAR, var_obs_noi))

%plot cross-spectrum matrix - MLE Q
figure
tiledlayout(1,numStates)
for i=1:numStates
    nexttile
    n = numNodes;
    imagesc(abs(S_y1y2_7hz_MLEQ(:,:,reorder(i))))
    yticks(1:n); xticks(1:n);
    set(gca, 'YTickLabel', 1:n)
    set(gca, 'XTickLabel', 1:n)
    colorbar
...
sgtitle(sprintf('Cross-spectrum: MLE \\Sigma (\\alpha=%g, var(R)=%g)', aAR, var_obs_noi))
...
for i=1:N
    ...
    for j = 1:3
        weighted_mat = weighted_mat + S_y1y2_7hz_MLEQ(:,:,j) * wt(j);
    end
