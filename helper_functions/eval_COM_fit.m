...
% ** first, load results from EM on B **
...
for st = 1:numStates
    ...
    CohMLEB(:, :, st) = abs(coherenceMatrix);
end
S_y1y2_7hz_MLEB = squeeze(S_y1y2(:,:,freqIndex,:));

% plot coherence matrix - MLE B
figure
tiledlayout(1,numStates)
for i=1:numStates
    nexttile
    n = numNodes;
    imagesc(CohMLEB(:,:,reorder(i)))
...
sgtitle(sprintf('Coherence: MLE B (\\alpha=%g, var(R)=%g)', aAR, var_obs_noi))


%plot cross-spectrual matrix - MLE B
figure
tiledlayout(1,numStates)
for i=1:numStates
    nexttile
    n = numNodes;
    imagesc(abs(S_y1y2_7hz_MLEB(:,:,reorder(i))))
    yticks(1:n); xticks(1:n);
    set(gca, 'YTickLabel', 1:n)
    set(gca, 'XTickLabel', 1:n)
    colorbar
...
sgtitle(sprintf('Cross-spectrum: MLE B (\\alpha=%g, var(R)=%g)', aAR, var_obs_noi))
...
for i=1:N
    ...
    for j = 1:3
        weighted_mat = weighted_mat + S_y1y2_7hz_MLEB(:,:,j) * wt(j);
    end
